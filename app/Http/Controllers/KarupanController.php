...
class KarupanController extends Controller
{
    ...
    public function update_karupan(Request $request,$asset_id)
    {
        ...
        $updated = DB::table('asset_main')->where('asset_id', $asset_id)->update($data);
    
        // ตรวจสอบว่าอัพเดตสำเร็จหรือไม่
        if ($updated) {
            return response()->json(['message' => 'Update successful for asset_id: ' . $asset_id]);
        } else {
            return response()->json(['message' => 'Update failed for asset_id: ' . $asset_id], 500);
        }
    }
